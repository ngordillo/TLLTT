...
import coast_experiment_settings
...
for coast_point in all_coast_points:
    ...
    loc_lat = int(coast_point[0])
    loc_lon = int(coast_point[1])

    EXP_NAME = 'loc_'+str(loc_lon)+'_'+str(loc_lat)+'_precip_5mean_5back'#'smaller_test'#'quadrants_testcase'

    imp.reload(coast_experiment_settings)
    settings = coast_experiment_settings.get_settings(EXP_NAME)
    ...
    labels, data, lat, lon, time = data_functions_schooner.load_tropic_data(DATA_DIR, loc_lon, loc_lat, True)
