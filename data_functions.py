...
def get_and_process_pres_data(raw_labels, raw_data, raw_time, rng, colored=False, standardize=False, shuffle=False):
...
def get_raw_temp_data(load_dir):
    ...
    # temp_label = np.loadtxt("/Users/nicojg/Documents/Work/2021_Fall_IAI/Code/TLLTT/data/tempclass_200year.txt")
    ...
    full_loc_temp = []
    #Loop by day first.
    # for i in np.arange(0,train_years,1):
    #     temp_loc = []
        
    #     #Then loop through the number of specified years. 
    #     for j in np.arange(0,365,1):

    #         #Add up the sea level pressure for that particular day from every year.
    #         temp_loc = temp[int(j+(days*i)),46,136]

    #         temp_loc -= avgtemp_day[j ,46,136]
